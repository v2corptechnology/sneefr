...
class YelpClient
{
     ...
     * @var Array
     */
    private static $defaultOptions = ['radius' => 500, 'limit' => 50, 'offset' => 0, 'categories' => 'shopping'];
    ...
    public static function getBusinessesAround(float $latitude, float $longitude, array $options = []) : Collection
    {
        ...
        $url = "businesses/search?latitude={$latitude}&longitude={$longitude}&" . http_build_query($options);

         return collect(json_decode((string) self::getClient()->get($url)->getBody(), true));
    }
    ...
    public static function getToken() : string
    {
        ...
        return cache()->remember('yelp_token', 2, function () use ($client) {
    ...
    {
        $randItem = array_rand(self::$tokens);

        return [
            'grant_type'    => 'client_credentials',
            'client_id'     => $randItem,
            'client_secret' => self::$tokens[$randItem],
        ];
    }
